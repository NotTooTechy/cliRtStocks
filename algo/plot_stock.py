...
trace2 = go.Scatter(
                    x=df['Date'], y=df['Adj Close'].rolling(window_size).mean(), # Data
...
for index, row in df.iterrows():
    ...
    moving_avg=df['Adj Close'].rolling(window=window_size, min_periods=1, center=False).mean()
    ...
    if count >30 and isinstance(moving_avg, float):
        if close_val> moving_avg and buy_flag:
            ...
            x = dict( x=row['Date'], y=row['Adj Close'], xref='x', yref='y', text='BUY', showarrow=True, arrowhead=5, ax=-30, ay=70, font=font_buy)
        ...
        if under_avg_count > 7:
            ...
            x = dict( x=row['Date'], y=row['Adj Close'], xref='x', yref='y', text='SELL', showarrow=True, arrowhead=2, ax=-30, ay=70, font=font_sell)
            annotations.append(x)
    count += 1
#annotations = [dict( x=row['Date'], y=row['Adj Close'], xref='x', yref='y', text='SELL', showarrow=True, arrowhead=7, ax=10, ay=-40)]
