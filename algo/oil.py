...
def sma_return(ticker, short_window, INITIAL_CAPITAL=17.0*1000.0, step_buy_th=STEP_BUY_THERESHOLD, step_sell_th=STEP_SELL_THRESHOLD):
	...
	signals['signal'] = 0.0
	signals['close'] = df['Last'].rolling(window=1, min_periods=1, center=False).mean()
	signals['volume'] = df['Volume'].rolling(window=1, min_periods=1, center=False).mean()
	signals['short_mavg'] = df['Last'].rolling(window=short_window, min_periods=1, center=False).mean()
	signals['short_max_avg'] = df['Last'].rolling(window=short_window, min_periods=1, center=False).max()
	#signals['long_mavg'] = instr['Close'].rolling(window=long_window, min_periods=1, center=False).mean()
	signals['long_mavg'] = df['Last'].rolling(window=5, min_periods=1, center=False).mean()
