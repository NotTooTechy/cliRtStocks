...
def sma_return(ticker, short_window, INITIAL_CAPITAL=17.0*1000.0, step_buy_th=STEP_BUY_THERESHOLD, step_sell_th=STEP_SELL_THRESHOLD):
	...
	signals['short_max'] = instr['Close'].rolling(window=20, min_periods=1, center=True).max()
	...
	for i in range(size):
		...
		short_max = round(signals['short_max'][i], 3)
		long_mavg = round(signals['long_mavg'][i], 3)
		if fprint:
			print date.date(), close, short_mavg, short_max,
